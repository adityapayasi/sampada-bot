...
os.environ["GEMINI_API_KEY"] = "demo-key-not-real"
...
from modules.ai_parser import (
    _clean_id_data, _normalise_date, _normalise_gender,
    _normalise_id_type, _extract_json, parse_id_card, parse_property_document
)
...
with patch("modules.db_manager.DB_PATH", TMP_DB):
    ...
    for lg in logs:
        icon = "OK" if lg["status"] == "success" else "PAUSE"
    ...
    print("-" * 60)
    # Create a fake image
    img_path = str(Path(tempfile.mkdtemp()) / "fake_id.png")
    ...
    with patch("modules.ai_parser.genai.GenerativeModel", return_value=mock_model):
    ...
    print(f"Parsed property document (mock response):")
    print(f"  District: {result2['district']}")
    print(f"  Plot:     {result2['plot_number']}")
    print(f"  Area:     {result2['total_area_sqmt']} sqmt")
    print(f"  East:     {result2['boundaries']['east']}")
    print()
